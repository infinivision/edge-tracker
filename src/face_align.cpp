...
//
//    https://github.com/szad670401/Umeyama-Similiar-Transfrom-for-face-alignment/blob/master/process.h
//    """
//    
//    Anthor: Jack Yu
cv::Mat similarTransform(cv::Mat src,cv::Mat dst) {
  ...
  if (rank == 0) {
    ...
    if (cv::determinant(U) * cv::determinant(V) > 0) {
      ...
      int s = d.at<float>(dim - 1, 0) = -1; // s = d[dim - 1]
      d.at<float>(dim - 1, 0) = -1;         // d[dim - 1] = -1

      T.rowRange(0, dim).colRange(0, dim) = U * V; // T[:dim, :dim] = np.dot(U, np.dot(np.diag(d), V))
      cv::Mat diag_ = cv::Mat::diag(d);            // d[dim - 1] = s
